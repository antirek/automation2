...
class StepWorker {
  ...
  async getVarsFromStore (stepTask) {
    ...
    if (varsIn && Array.isArray(varsIn)) {
      for (const varIn of varsIn) {
        data[varIn] = await this.store.getData(stepTask.taskId, varIn);
      }
    }
    return data;
  }
  ...
  async setVarsToStore (stepTask, data) {
    ...
    if (varsOut && Array.isArray(varsOut)) {
      for (const varOut of varsOut) {
        const stringData = data[varOut];
        console.log('>>>>>>> ', stepTask.taskId, varOut, stringData);
        await this.store.setData(stepTask.taskId, varOut, stringData);
      }
    }
  }
  ...
  async process (job) {
    ...
    await this.postprocess(stepTask, result);
  }
}
